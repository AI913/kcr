...
class ActionEntryConfigurationPresenterTests: XCTestCase {

    private let ms1000 = 1.0
    private let vc = ActionEntryConfigurationViewControllerProtocolMock()
    private let useCase = JobOrder_Domain.DataManageUseCaseProtocolMock()
    private let viewData = MainViewData.Job()
    private lazy var presenter = ActionEntryConfigurationPresenter(useCase: useCase, vc: vc, viewData: viewData)
