...
extension ActionEntryConfigurationPageViewController: UIPageViewControllerDelegate {
    ...
    func pageViewController(_ pageViewController: UIPageViewController,
        ...
        if let vc = pendingViewControllers.first as? ActionEntryConfigurationContainerViewController,
    ...
    func pageViewController(_ pageViewController: UIPageViewController,
        ...
        if completed, let vc = pageViewController.viewControllers?[0] as? ActionEntryConfigurationContainerViewController,
           let index = controllers.firstIndex(of: vc) {
            nowIndex = index
            self.preferredContentSize.height = vc.preferredContentSize.height
        } else if let vc = previousViewControllers.first as? ActionEntryConfigurationContainerViewController,
                  let index = controllers.firstIndex(of: vc) {
            _delegate?.pageChanged(index: index)
        }
    }
...
extension ActionEntryConfigurationPageViewController: UIPageViewControllerDataSource {
    ...
    func pageViewController(_ pageViewController: UIPageViewController, viewControllerBefore viewController: UIViewController) -> UIViewController? {
        if let index = controllers.firstIndex(of: viewController as! ActionEntryConfigurationContainerViewController),
           index > 0 {
