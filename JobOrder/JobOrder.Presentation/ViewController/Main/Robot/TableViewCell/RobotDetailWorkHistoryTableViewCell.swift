...
class RobotDetailWorkHistoryTableViewCell: RobotDetailWorkTableViewCell {
    ...
    override func setRow(_ indexPath: IndexPath) {
        jobNameValueLabel.text = presenter?.jobName(indexPath.section)
        identifierValueLabel.attributedText = presenter?.queuedAt(in: dataset, indexPath.section, textColor: lastUpdateAtValueLabel.textColor, font: lastUpdateAtValueLabel.font)
        lastUpdateAtValueLabel.text = presenter?.resultInfo(in: dataset, indexPath.section)
